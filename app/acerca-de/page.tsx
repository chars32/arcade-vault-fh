...
// ===== app/acerca-de/page.tsx — Acerca de + contacto =====
...
export default function AcercaDePage() {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    if (!form.name.trim() || !form.email.trim() || !form.msg.trim() || !EMAIL_RE.test(form.email)) {
      e.preventDefault();
      setShake(true);
          ...
          <form className={"contact-form" + (shake ? " shake" : "")} action={formAction} onSubmit={onSubmit}>
            {!showSuccess ? (
              <>
                ...
                {state.status === "error" && <div className="contact-error">{state.message}</div>}
              </>
            ) : (
